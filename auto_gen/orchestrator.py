...
def check_worktree_has_changes(worktree_path: str, operator: str, metax: bool = False, iluvatar: bool = False) -> bool:
...
def parse_cc_result(proc: subprocess.Popen, operator: str, worktree_path: str = None, metax: bool = False, iluvatar: bool = False) -> dict:
    ...
    try:
        ...
        if proc.returncode == 0 and worktree_path and check_worktree_has_changes(worktree_path, operator, metax=metax, iluvatar=iluvatar):
...
def run(args):
    ...
    # Determine if running in metax or iluvatar mode
