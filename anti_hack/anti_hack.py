...
def disable_triton_jit():
    ...
    Also clears LibEntry kernel caches to prevent warm-cache reuse
    of previously compiled kernels from the normal run.
    """
    try:
    ...
    # Clear all LibEntry kernel caches so the disabled run cannot reuse
    # compiled kernels cached by the normal (pre-disabled) execution.
    _clear_libentry_caches()
    try:
        yield
    finally:
...
def _clear_libentry_caches():
    """Clear kernel caches in all LibEntry instances reachable via flag_gems."""
    try:
        from flag_gems.utils.libentry import DEVICE_COUNT
        empty_cache = tuple(dict() for _ in range(DEVICE_COUNT))
        import gc
        for obj in gc.get_objects():
            try:
                cls = obj.__class__
                if cls.__name__ == "LibEntry":
                    obj.kernel_cache = tuple(dict() for _ in range(DEVICE_COUNT))
                    obj._cpu_cache = dict()
            except Exception:
                pass
    except Exception:
...
def dual_execution_check(
    ...
    # Run 1: normal execution (clone input tensors to preserve originals)
    kwargs_normal = _clone_tensor_kwargs(kwargs)
    try:
        out_normal = func(**kwargs_normal)
    except Exception:
        # If normal run fails, can't do comparison
        return False, ""

    # Run 2: with triton.jit disabled
    kwargs_disabled = _clone_tensor_kwargs(kwargs)
    try:
        with disable_triton_jit():
            out_disabled = func(**kwargs_disabled)
    except Exception:
...
def _clone_tensor_kwargs(kwargs: dict) -> dict:
    """Clone tensor values in kwargs so inplace ops don't affect subsequent runs."""
    import torch
    cloned = {}
